...
const categories = ["Все","Автомобили","Недвижимость","Работа","Услуги","Бизнесы","Сим-Карты","Другое"];
...
const carGroups = [
  ...
  { title: "Уникальные автомобили", items: [
    "Ambulance","BMW M5 F90","Coach","Dodge Challenger SRT-8","Dozer","Duneride","Dumper","Enforcer","FBI Rancher","FBI Truck","Fire Truck","Fire Truck LA","Glendale Shit","Mesa","Monster Truck","Mazda RX-8","Newsvan","Patriot","Packer","Police Car (LS/SF/LV)","Police Ranger","Roadtrain","Taxi","Tractor","Trashmaster"
  ]},
...
const search=ref(""); const activeCategory=ref("Все"); const activeCity=ref("Все"); const activeDealType=ref("Все");
...
const form=ref({title:"",category:"Автомобили",dealType:"Продажа",city:"Los Santos",price:"",seller:"",contact:"",description:"",vk:"",tuning:"",accelerationStage:"",speedStage:"",district:"",houseNumber:""});
...
const filteredAds=computed(()=>{
  ...
  const result=ads.value.filter(ad=>{
    ...
    return (!q||text.includes(q)) && (activeCategory.value==="Все"||ad.category===activeCategory.value) && (activeCity.value==="Все"||ad.city===activeCity.value) && (activeDealType.value==="Все" || (activeDealType.value==="Куплю" ? /^Куплю\s+/i.test(ad.title) : /^Продам\s+/i.test(ad.title))) && ad.price>=min && ad.price<=max;
  });
...
function applyDealPrefix(value:string, dealType:string, category:string){
  const base=stripDealPrefix(value);

  if(category==="Сим-Карты"){
    return `${dealPrefix(dealType)} сим-карту ${base}`.trim();
  }

  if(["Автомобили","Недвижимость","Бизнесы"].includes(category)){
    return `${dealPrefix(dealType)} ${base}`.trim();
  }
...
function rebuildAutoDescription(){
  ...
  if(form.value.category==="Автомобили"){
    ...
    if(form.value.category==="Недвижимость" && form.value.houseNumber) lines.push(`Номер дома - ${form.value.houseNumber};`);
...
function resetCreateForm(){
  form.value={title:"",category:"Автомобили",dealType:"Продажа",city:"Los Santos",price:"",seller:"",contact:"",description:"",vk:"",tuning:"",accelerationStage:"",speedStage:"",district:"",houseNumber:""};
...
function openEdit(ad:Ad){
  ...
  editForm.value={
    title:stripDealPrefix(ad.title),
    category:ad.category,
    dealType,
    city:ad.city,
    price:ad.price,
...
function rebuildEditDescription(){
  ...
  if(editForm.value.category==="Автомобили"){
    ...
    if(editForm.value.category==="Недвижимость" && editForm.value.houseNumber){
      lines.push(`Номер дома - ${editForm.value.houseNumber};`);
    }
...
async function saveEdit(){
  ...
  if(editForm.value.category==="Автомобили"){
    ...
    if(editForm.value.category==="Недвижимость" && editForm.value.houseNumber) lines.push(`Номер дома - ${editForm.value.houseNumber};`);
  ...
  const payload={...editForm.value,title:applyDealPrefix(editForm.value.title,editForm.value.dealType,editForm.value.category),description,vk:normalizedVk};
  ...
  <form class="modal" @submit.prevent="submitAdForm">
    ...
    <template v-else-if="form.category==='Недвижимость'">
      ...
      <div style="margin-top: 16px;">
        <label style="font-size: 13px !important; font-weight: 600 !important; color: #8b909b !important; margin-bottom: 6px !important; display: block !important;">Номер дома</label>
        <input v-model="form.houseNumber" @input="rebuildAutoDescription" maxlength="20" placeholder="Например: 125" style="font-size: 13px !important;">
      </div>
    </template>
    ...
    <template v-else-if="form.category==='Сим-Карты'">
      <div style="margin-top: 16px;">
        <label style="font-size: 13px !important; font-weight: 600 !important; color: #8b909b !important; margin-bottom: 6px !important; display: block !important;">Формат SIM-карты / номер</label>
        <input v-model="form.title" required maxlength="80" placeholder="Например: XXX, XYX или обычная SIM" style="font-size: 13px !important;">
      </div>
    </template>
    ...
    <template v-else-if="form.category==='Другое'">
      <div style="margin-top: 16px;">
        <label style="font-size: 13px !important; font-weight: 600 !important; color: #8b909b !important; margin-bottom: 6px !important; display: block !important;">Название</label>
        <input v-model="form.title" required maxlength="80" placeholder="Напишите название вручную..." style="font-size: 13px !important;">
      </div>
    </template>
...
<div v-if="editAd" class="modal-backdrop modal-form-backdrop" @click.self="editAd=null">
  <div class="modal-scroll">
...
<template v-else-if="editForm.category==='Недвижимость'">
  ...
  <label style="margin-top: 12px;">Номер дома<input v-model="editForm.houseNumber" @input="rebuildEditDescription" maxlength="20" placeholder="Например: 125"></label>
</template>
...
<template v-else-if="editForm.category==='Сим-Карты'">
  <div style="margin-top: 12px;">
    <label>Формат SIM-карты / номер<input v-model="editForm.title" required maxlength="80" placeholder="Например: +7 900 123-45-67 или обычная SIM"></label>
  </div>
</template>
